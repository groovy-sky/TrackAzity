...
az deployment group create --resource-group $container_name --template-file Container.json --parameters storageName=$storage_account_name hubResourceID=$hub_vnet_id devOpsOrg=$devops_org devOpsWebhook=$devops_webhook 